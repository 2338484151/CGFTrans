...
class AttModel(CaptionModel):
    ...
    def get_logprobs_state(self, it, att_feats, goabl_h, att_masks, state, output_logsoftmax=1):
        # 'it' contains a word index
        xt = self.embed(it)
        output, state = self.core(xt, att_feats, goabl_h, state, att_masks)
    ...
    def _sample_beam(self, att_feats, att_masks=None, opt={}):
        ...
        p_att_feats, pp_att_feats, goabl_h, p_att_masks = self._prepare_feature(att_feats, att_masks)
        ...
        logprobs, state = self.get_logprobs_state(it, pp_att_feats, goabl_h, p_att_masks, state)

        pp_att_feats, goabl_h, p_att_masks = utils.repeat_tensors(beam_size, [pp_att_feats, goabl_h, p_att_masks])
        self.done_beams = self.beam_search(state, logprobs, pp_att_feats, goabl_h, p_att_masks, opt=opt)
    ...
    def _sample(self, att_feats, att_masks=None, update_opts={}): # 测试调用这里
        ...
        if beam_size > 1 and sample_method in ['greedy', 'beam_search']:
            return self._sample_beam(att_feats, att_masks, opt)
        if group_size > 1:
